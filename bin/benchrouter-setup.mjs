...
async function init() {
  ...
  printInitNextSteps({ hasSetupAgent: packet.files.some((file) => file.path === ".benchrouter/SETUP_AGENT.md") });
...
function printInitNextSteps({ hasSetupAgent }) {
  process.stdout.write("\nNext steps:\n");
  if (hasSetupAgent) {
    process.stdout.write("- Tell your coding agent: read .benchrouter/SETUP_AGENT.md before editing.\n");
  } else {
    process.stdout.write("- If .benchrouter/SETUP_AGENT.md was generated by this packet, tell your coding agent to read it before editing.\n");
  }
  process.stdout.write("- Choose the case source branch: test-derived if existing tests/mocks already assert the model output; otherwise captured. For test-derived, fill cases from fixtures/assertions and keep the fail-closed scorer until it is extracted. For captured, use the sidecar/capture flow once.\n");
  process.stdout.write("- Patch exactly one runtime call site to use the BenchRouter route ID as that call site's model value; record call_site.base_url_env and code_refs in .benchrouter/benchrouter.yml.\n");
  process.stdout.write("- Install secrets after user approval: runtime host BENCHROUTER_API_KEY in the app runtime, GitHub Actions repo secret BENCHROUTER_EVAL_API_KEY for evals.\n");
